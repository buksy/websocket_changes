...
public final class WebSocketImpl extends WebSocket {
	...
	public SocketClient sockchannel;
...
//	public SelectionKey key;
	 ...
	 *            they occur.
	 */
	public WebSocketImpl( WebSocketListener listener , Draft draft , SocketClient socketchannel ) {
		init( listener, draft, socketchannel );
	}

	public WebSocketImpl( WebSocketListener listener , List<Draft> drafts , SocketClient socketchannel ) {
	...
	private void init( WebSocketListener listener, Draft draft, SocketClient socketchannel ) {
	...
	public boolean read( ByteBuffer buf ) throws IOException {
		...
		int read = sockchannel.getInputStream().read(b, 0 , buf.capacity());
//		System.out.println(new String(b) + read);
//		if(read==161) {
//			System.out.println((int)b[160]);
//			read += sockchannel.getInputStream().read(b, read , buf.capacity() - read);
//		}
		buf.put(b);
		buf.limit(read);
		buf.flip();
	 ...
	 */
	@Override
	public void send( ByteBuffer bytes ) throws IllegalArgumentException , NotYetConnectedException , InterruptedException, IOException {
		if( bytes == null )
			throw new IllegalArgumentException( "Cannot send 'null' data to a WebSocketImpl." );
	...
	@Override
	public void send( byte[] bytes ) throws IllegalArgumentException , NotYetConnectedException , InterruptedException, IOException {
		send( ByteBuffer.wrap( bytes ) );
	}

	private void send( Collection<Framedata> frames ) throws IOException {
	...
	@Override
	public void sendFrame( Framedata framedata ) throws IOException {
		if( DEBUG )
...
//		while ( buffer != null ) {
//			int written = sockchannel.getOutputStream().write(arg0)( buffer );
...
//	}
	...
	@Override
	public void startHandshake( ClientHandshakeBuilder handshakedata ) throws InvalidHandshakeException, IOException {
		if( handshakeComplete )
	...
	private void channelWrite( ByteBuffer buf ) throws IOException {
		sockchannel.getOutputStream().write(buf.array());
		sockchannel.getOutputStream().flush();
	}
	...
	public InetSocketAddress getRemoteSocketAddress() {
		return (InetSocketAddress) sockchannel.getRemoteAddress();
	}

	public InetSocketAddress getLocalSocketAddress() {
		return (InetSocketAddress) sockchannel.getLocalAddress();
	}
